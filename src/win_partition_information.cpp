...
void NativePartititonInformation::collect_partititon_information()
{
    ...
    while (drive_mask) {
        if (drive_mask & 0x1) {
            ...
            size_t disk_pattern_size = sizeof(disk_windows_pattern);
            int converted = MultiByteToWideChar(CP_ACP, 0, disk_windows_pattern, disk_pattern_size, disk_to_unicode, disk_pattern_size);
            if (0 == converted) {
                return;
            }
            std::wstring wdisk_windows_pattern(disk_to_unicode);
