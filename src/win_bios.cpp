...
#include <winapi_helpers/win_bios.h>
...
SMBiosImpl::SMBiosImpl() : native_system_information_(std::make_unique<helpers::NativeSystemInformation>())
{
...
size_t SMBiosImpl::get_table_size() const
{
    ...
    if (smbios_data_) {
        return static_cast<size_t>(smbios_data_->length);
    }
    else {
...
{
    FARPROC system_firmware_call = GetProcAddress(GetModuleHandleA("kernel32.dll"), "GetSystemFirmwareTable");
...
void SMBiosImpl::compose_native_smbios_table()
{
    ...
    if (is_upto_vista && is_ntdll_compatible()) {
        ...
        DWORD sys_smbios_table_size = static_cast<DWORD>(smbios_table_size);
        GetSystemFirmwareTable('RSMB', 0, &table_buffer_[0], sys_smbios_table_size);
