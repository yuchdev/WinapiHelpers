...
#include <winapi_helpers/win_physical_memory.h>
#include <winapi_helpers/win_native_api_helper.h>
...
void NativePhysicalMemory::map_physical_memory(size_t base, size_t length)
{
    ...
    page_offset_ = base % static_cast<size_t>(sysinfo.dwPageSize);

    // Map memory here
    PHYSICAL_ADDRESS physical_base{};
    physical_base.QuadPart = static_cast<ULONGLONG>(base);
    ULONG sys_defined_length = static_cast<ULONG>(length);
    NTSTATUS map_view_status = NtMapViewOfSection(
        ...
        0L,                                     // ZeroBits
        sys_defined_length,                     // CommitSize
        &physical_base,                         // SectionOffset
        &sys_defined_length,                    // ViewSize
...
bool NativePhysicalMemory::is_ntdll_compatible() const
{
    ...
    FARPROC rtl_init_unicode_string = GetProcAddress(GetModuleHandleA("ntdll.dll"), "RtlInitUnicodeString");
    if (RtlInitUnicodeString != reinterpret_cast<void*>(rtl_init_unicode_string)) {
        return false;
    }

    // NtMapViewOfSection
    FARPROC map_view_section = GetProcAddress(GetModuleHandleA("ntdll.dll"), "NtMapViewOfSection");
    if (NtMapViewOfSection != reinterpret_cast<void*>(map_view_section)) {
        return false;
    }

    // NtUnmapViewOfSection
    FARPROC unmap_view_section = GetProcAddress(GetModuleHandleA("ntdll.dll"), "NtUnmapViewOfSection");
    if (NtUnmapViewOfSection != reinterpret_cast<void*>(unmap_view_section)) {
        return false;
    }

    // NtOpenSection
    FARPROC open_section = GetProcAddress(GetModuleHandleA("ntdll.dll"), "NtOpenSection");
